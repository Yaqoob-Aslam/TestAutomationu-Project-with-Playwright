...
test.afterEach(async () => {
    try {
        ...
        if ((err as any).name === 'DiffsFoundError') {
            console.warn('Visual differences detected. See Applitools dashboard.');
        } else {
            throw err; // other errors should fail the test
        }
    }
});
...
test.afterAll(async () => {
    try {
        const results = await Runner.getAllTestResults(false); // false = don’t throw on diff
        console.log('Visual test results', results);
    } catch (err) {
        console.error('Error fetching visual test results', err);
    }
});
